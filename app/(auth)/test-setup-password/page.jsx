...
export default function SetupPasswordPage() {
  ...
  const { mutate: activateAccount, isPending: isActivating } = useMutation({
    mutationFn: async (payload) => {
      const response = await api.post("/verify-employee", payload);
      return response.data;
    },
    onSuccess: (data) => {
      ...
      setIsSuccess(true);
      setTimeout(() => {
        router.push("/login"); // Password set pannathum login-ku anuppalaam
      }, 2500);
    },
    onError: (error) => {
        ...
        >
          ...
          <div className="relative">
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              transition={{ delay: 1 }}
              className="mt-8 flex justify-center items-center gap-2 text-slate-400 text-xs uppercase tracking-[0.2em] font-bold"
            >
